...
def get_R_based_on_c(train_filepaths, train_labels, nu, eps = 1e-4):

    latent_features_list = []

    for index in range(len(train_filepaths)):
        if(train_labels[index] == 0):
            image = get_image(train_filepaths[index])
...
# add tensorboard
def train(train_filepaths, train_labels, validation_filepaths, validation_labels, epochs, BATCH_SIZE, nu):
    ...
    for epoch in range(epochs):
        ...
        for index in range(0, len(train_labels)-BATCH_SIZE, BATCH_SIZE):
            ...
            if(iteration % 30 == 0):
                ...
                R = get_R_based_on_c(train_filepaths, train_labels, nu)
                ...
                for val_index in range(0, len(validation_labels)-BATCH_SIZE, BATCH_SIZE):
                    ...
                    loss, metric_results, val_predictions = valid_step(image_batch, label_batch, R)
...
def evaluation(train_filepaths, train_labels, test_filepaths, test_labels, tag_name, nu):
...
train(train_filepaths, train_labels, validation_filepaths, validation_labels, epochs, BATCH_SIZE, nu)
evaluation(train_filepaths, train_labels, validation_filepaths, validation_labels, "validation", nu)
evaluation(train_filepaths, train_labels, test_filepaths, test_labels, "testing", nu)
