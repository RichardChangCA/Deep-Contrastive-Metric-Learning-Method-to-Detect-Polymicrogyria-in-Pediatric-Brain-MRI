...
train_images, train_labels, validation_images, validation_labels, test_images, test_labels = dataset_collection_func(normal_class = 3, abnormal_classes_list = [4,5,6,7,8], abnormal_ratio = 0.1, normal_amount=5000)
...
def pumad_inference(train_images, train_labels, validation_images, validation_labels, test_images, test_labels):
    print("pumad inference")
    checkpoint.restore(ckpt_manager.latest_checkpoint)
    ...
    for i in range(len(train_labels)):
        ...
        train_normal_predictions.append(prediction)

    train_normal_predictions = np.array(train_normal_predictions)

    train_normal_predictions = train_normal_predictions.squeeze()
    ...
    for i in tqdm(range(len(test_labels))):
        ...
        for normal_idx in range(len(train_normal_predictions)):
            distance_list.append(np.sum((train_normal_predictions[normal_idx] - prediction)**2))

        distance_list = np.array(distance_list)

        avg_distance = np.sum(distance_list) / len(train_normal_predictions)

        test_values.append(avg_distance)

    test_values = np.array(test_values)

    auc_roc = sklearn.metrics.roc_auc_score(test_labels, test_values)

    print("auc_roc:", auc_roc)

pumad_inference(train_images, train_labels, validation_images, validation_labels, test_images, test_labels)
